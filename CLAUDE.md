...
│   │   │   ├── documents.ts  # uploadDocumentMultiAction (1 doc/PDF unificado, multi-firmante con campos[]) — lo usa el wizard; uploadDocumentsBatchAction (lote de docs independientes) quedó sin uso; resend/update por firmante, hideDocumentAction
...
- **Posicionador — render serializado + zoom + multi-campo**: `renderPage` **cancela** la tarea de render anterior (`renderTaskRef`) antes de iniciar otra — sin esto, dos `render()` sobre el mismo canvas chocan y dejan el preview en blanco. El `ResizeObserver` omite su primer disparo. Hay **zoom** (estado `zoom` 0.6–2.5, `zoomRef`; contenedor `overflow-auto max-h-[80vh]`). **Multi-campo**: **clic** en el PDF AGREGA un recuadro para el firmante activo (`onAddField`); cada recuadro se **arrastra** (`onMoveField`, pointer events mouse+touch) y se **quita** con la ✕ (`onRemoveField`). **Cualquier** recuadro se puede tomar con clic — selecciona a su firmante (esto arregló "no podía mover al firmante 1 con el 2 activo", que causaba firmas encimadas → la del firmante 2 quedaba tapada). El estado por firmante muestra el conteo de campos.
