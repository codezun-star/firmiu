...
- **Subida por lote (varios documentos)**: `uploadDocumentsBatchAction` (FormData `count` + `pdf_{i}`/`firmantes_{i}`). Cada documento es **independiente** (su propio PDF, firmantes y posiciones). Comparte helpers con la subida de 1 doc: `validateDocInput`, `resolvePlanLimits`, `createOneDocument` (este NO chequea plan ni incrementa contador — lo hace el caller, por eso el contador sube por documento creado). Tope por carga = `BATCH_LIMITS` (free 1 · starter 5 · pro 20 · business 50) **∩** cupo mensual restante; valida TODOS los docs antes de crear ninguno. `NuevoForm` mantiene un "tray" de borradores: el asistente de 3 pasos configura un doc, "Agregar otro documento" lo guarda y reinicia; el envío manda `[tray..., actual]`. La página `/dashboard/nuevo` (server, ahora `async`) calcula `maxBatch` y lo pasa al form. Free = `maxBatch` 1 → comportamiento idéntico al anterior (single doc).
