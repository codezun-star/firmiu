...
- **Subida por lote (varios documentos)**: `uploadDocumentsBatchAction` (FormData `count` + `pdf_{i}`/`firmantes_{i}`/`modo_{i}`). Cada documento es **independiente** (su propio PDF, firmantes, posiciones y modo de firma). Comparte helpers con la subida de 1 doc: `validateDocInput`, `resolvePlanLimits`, `createOneDocument` (este NO chequea plan ni incrementa contador — lo hace el caller, por eso el contador sube por documento creado). Tope por carga = `BATCH_LIMITS` (free 1 · starter 5 · pro 20 · business 50) **∩** cupo mensual restante; valida TODOS los docs antes de crear ninguno. En `NuevoForm` el step 0 admite **selección múltiple** (input `multiple` cuando `maxBatch>1`): el primer archivo es el "actual" y el resto va a una `queue`; el asistente configura uno (posición + firmantes + orden) y "Siguiente documento" pasa al siguiente de la cola; los ya configurados van a un "tray". La página `/dashboard/nuevo` (server, `async`) calcula `maxBatch` y lo pasa al form. Free = `maxBatch` 1 → input single, comportamiento idéntico al anterior.
...
- **Posicionador (`/nuevo` paso 2) — layout y nitidez**: `PdfPosicionador.tsx` usa **dos paneles** (PDF grande a la izquierda + controles de firmante a la derecha) y el paso ocupa **todo el ancho** (`NuevoForm` pone `max-w-6xl` en el paso de posicionar y `max-w-5xl` + sidebar `whatNext` en los pasos 0/2). El canvas se renderiza a **devicePixelRatio** (interno = displayPx × dpr, CSS = displayPx) y se ajusta a ~80vh de alto → nítido en pantallas HiDPI (esto arregló el "segundo documento borroso") y muestra la página completa. Un `ResizeObserver` re-renderiza al cambiar el ancho. Los tabs de firmante viven DENTRO de `PdfPosicionador` (props `onSelectSigner`/`onAddSigner`). `NuevoForm` hace scroll suave al cambiar de paso (`firmiu-step-in` en globals.css) — single y multi-documento comparten exactamente la misma UI.
- **Posicionador — render serializado + zoom + arrastre**: `renderPage` **cancela** la tarea de render anterior (`renderTaskRef`) antes de iniciar otra — sin esto, dos `render()` sobre el mismo canvas chocan y dejan el preview en blanco (era el "a veces no se previsualiza"). El `ResizeObserver` omite su primer disparo. Hay **zoom** ajustable (estado `zoom` 0.6–2.5, `zoomRef` para no recrear `renderPage`; el contenedor es `overflow-auto max-h-[80vh]`). Posicionar = **clic** en el PDF (coloca al firmante activo) o **arrastrar** el recuadro del firmante activo (pointer events, mouse+touch). NO hay presets de posición (se quitaron). El auto-avance al siguiente firmante en `handlePlace` ocurre SOLO en la primera colocación (`wasUnplaced`) para no saltar de firmante al arrastrar.
- **Importar firmantes desde contactos**: `/nuevo` (server) trae `contactos` (tabla `contactos`, `oculto=false`) y los pasa a `NuevoForm`. En el paso de firmantes, cada tarjeta tiene "Desde contactos" → dropdown con búsqueda → rellena nombre+correo del firmante. Solo lectura (no crea contactos).
